...
always@*
	...
	WA_VALID_M:		if(AWREADY)			begin WANext_state_M = WA_ADDR_M;  $display("WA_VALID_M to WA_ADDR_M%t",$time); end
    				else				begin WANext_state_M = WA_VALID_M; $display("Back to WA_VALID_M from WA_VALID_M %t",$time); end

    WA_ADDR_M:							begin WANext_state_M = WA_WAIT_M;  $display("WA_ADDR_M to WA_WAIT_M %t",$time); end
								
	WA_WAIT_M:		if(BVALID)			begin WANext_state_M = WA_IDLE_M;  $display("WA_WAIT_M to WA_IDLE_M transaction done %t",$time); end
					else				begin WANext_state_M = WA_WAIT_M;  $display("Back to WA_WAIT_M from WA_WAIT_M %t",$time); end

	default :							begin WANext_state_M = WA_IDLE_M;  $display("Back to WA_IDLE_M from default %t",$time); end
...
always@*
	...
	R_IDLE_M  :    if(ARREADY && araddr != awaddr) RNext_state_M = R_READY_M;
            		                               ...
            		                               $display("CANNOT READ WRITE AT SAME LOCATION\n\
            		                                         %t, awaddr = %h,araddr = %h and ARREADY = %h",
            		                                         $time,awaddr,araddr,ARREADY); end
