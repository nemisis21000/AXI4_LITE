...
logic [DW/8-1:0] wstrb = '1;
logic [DW-1:0]   rdata;
logic             done;
logic             err;
logic [DW-1:0]   reg_out [0:N_REGS-1];
...
task automatic check_reg(
    ...
    if (reg_out[idx] === exp) begin
        $display("[PASS] %-35s  reg[%0d]=0x%08h", name, idx, reg_out[idx]);
        pass_cnt++;
    end else begin
        $display("[FAIL] %-35s  reg[%0d]=0x%08h  exp=0x%08h", name, idx, reg_out[idx], exp);
        fail_cnt++;
    end
endtask
